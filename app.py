...
# Schema for the frontend whale injection API
class WhaleAction(BaseModel):
    action: str
    volume: int

# Dueling architecture splits the value and advantage streams
# This stops the agent from thinking a lucky random walk was a genius strategic move
class DuelingDQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        super().__init__()
        # Base feature extraction from the LOB state
        ...
        # Evaluates the inherent danger of the current market state V(s)
        ...
        # Evaluates the relative benefit of a specific quote depth A(s,a)
    ...
    def forward(self, x):
        ...
        # Recombine streams while forcing the mean advantage to 0 for mathematical identifiability
        qvals = values + (advantages - advantages.mean(dim=1, keepdim=True))
        return qvals

class QuantEngineEnv:
    def __init__(self):
        # Base simulation mechanics
        self.cash = 10000.0
        self.inv = 0
        self.mid = 100.0
        self.obi = 0.0 # Order Book Imbalance SDE state
        self.tick_sz = 0.01
        self.limit = 100
        self.internal_tick = 0
        self.mid_history = deque(maxlen=20)
        
        # 5D state tensor -> 9 discrete spread combinations
        self.state_dim = 5
        self.action_dim = 9
        self.depths = [1, 5, 15]
        
        # Initialize primary and target networks for Double DQN logic
        ...
        self.criterion = nn.SmoothL1Loss() # Huber loss handles outliers better than pure MSE here
        ...
        self.tau = 0.005 # Polyak averaging factor for soft target updates
    ...
    def trigger_whale(self, direction, volume):
        if not self.liquidated:
            ...
            # Strict 5-second cooldown to prevent API spamming the tape
    ...
    # Packs the current market microstructure into a tensor-ready array
    def _get_state(self):
        # Calculate rolling momentum and micro-volatility if we have enough history
        ...
        # Note: Hardcoded multipliers here are a bit crude for normalisation, but it works for the sim
        return np.array([self.inv / self.limit, self.obi, self._get_cooldown_ratio(), momentum * 10.0, volatility * 10.0], dtype=np.float32)

    def _train(self):
        # Wait until we have enough replay memory to sample a full batch
        if len(self.memory) < self.batch_size: return 0.0
        
        batch = random.sample(self.memory, self.batch_size)
        states, actions, rewards, next_states, dones = zip(*batch)
        
        # Push everything to PyTorch tensors
        ...
        # Double DQN logic: Primary net picks the action, target net evaluates it
        with torch.no_grad():
        ...
        # Clip gradients to prevent explosive backprop if the market spikes
        torch.nn.utils.clip_grad_norm_(self.net.parameters(), 1.0)
        self.optimizer.step()
        
        # Soft update the target network weights using Polyak averaging
    ...
    def step(self):
        # Halt the engine if margin call was hit
        if self.liquidated:
        ...
        # Autocorrelated order flow simulation (Ornstein-Uhlenbeck process)
        ...
        # Forward pass to get Q-values and advantage stream for telemetry
        with torch.no_grad():
        ...
        # Map discrete action integer (0-8) to actual Bid/Ask depth ticks
        b_idx = action // 3
        a_idx = action % 3
        ...
        # Random walk the true mid price
        self.mid += np.random.normal(0.0, 0.05)
        trade_log = "-"
        
        # Process manual toxic flow injections from the frontend
        if self.whale_queue:
            direction, vol = self.whale_queue.pop(0)
            slip = (vol // 20) * self.tick_sz # Calculate slippage based on volume
            if direction == 'buy':
                fill = ask + (slip * 0.5)
            ...
            # Normal market making fill probabilities based on depth and current OBI
        ...
        # Calculate step PnL for reward shaping
        ...
        # Reward shaping: Optimize for Sortino ratio to penalize only downside volatility
        downside = [r for r in self.returns if r < 0]
        sortino = 0.0
        if downside:
            sortino = (np.mean(self.returns) / (np.std(downside) + 1e-5)) * math.sqrt(252 * 23400)
            
        # Avellaneda-Stoikov style quadratic inventory penalty
        inv_penalty = 0.5 * (self.inv / self.limit)**2
        
        # Combine components and clip to prevent weight destruction on outliers
        ...
        # Train the network every 5 ticks
        if self.internal_tick % 5 == 0:
            loss = self._train()
            if loss > 0: self.last_loss = loss
            
        # Basic XAI reasoning generation for the frontend
    ...
    # Formats the state data for JSON serialization over the API
...
def get_session_env(session_id: str) -> QuantEngineEnv:
    ...
    # Garbage Collector: Destroy any environments that haven't been pinged in 5 minutes
    # This prevents massive memory leaks in RAM if users close the tab
    expired_sessions = [sid for sid, data in active_sessions.items() if now - data.last_accessed > timedelta(minutes=5)]
    for sid in expired_sessions:
        del active_sessions[sid]
        
    # Spin up a new neural network environment if the tab is new
